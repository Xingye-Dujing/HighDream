...
def handle_fx_mul_exp_gx(expr: Expr, exp_term: exp, another_term: Expr, context: RuleContext) -> RuleFunctionReturn:
    ...
    step_gene.add_step(
        'None',
        f'$= \\int {latex(diff_m_x * exp_term)}\\,'
        f'\\mathrm{{d}} {var} + \\int {latex((another_term - diff_m_x) * exp_term)}\\,\\mathrm{{d}} {var}$')
    step_gene.add_step(
        'None',
        f'$= \\int {latex(diff_m_x * exp_term)}\\,'
        f'\\mathrm{{d}} {var} + \\int {latex(m_x)}\\,\\mathrm{{d}} {latex(exp_term)}$')
    step_gene.add_step(
        'None',
        f'$= \\int {latex(diff_m_x * exp_term)}\\,'
        f'\\mathrm{{d}} {var} + {latex(result)} - \\int {latex(diff_m_x * exp_term)}\\,\\mathrm{{d}} {var}$')
    step_gene.add_step(
